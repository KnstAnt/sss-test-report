...
impl Curve
{
    ...
    pub fn new_catmull_rom(src: &[(f64, f64)]) -> Result<Curve, Error> {
        ...
        let delta_value1 = (src[1].1 - src[0].1)/delta_key1;
        let delta_value2 = (src[2].1 - src[1].1)/delta_key2;
        let delta = (delta_value1 - delta_value2)/delta_key1;
        let mut add_value = |i: f64| {
            let delta_key = delta_key1 * i;
            res.push(Key::new(
                src[0].0 - delta_key,
                src[0].1 - (delta_value1 + delta*delta_key)*delta_key,
        ...
        let delta_value1 = (src[src.len() - 1].1 - src[src.len() - 2].1)/delta_key1;
        let delta_value2 = (src[src.len() - 2].1 - src[src.len() - 3].1)/delta_key2;
        let delta = (delta_value1 - delta_value2)/delta_key1;
        let mut add_value = |i: f64| {
            let delta_key = delta_key1 * i;
            res.push(Key::new(
                src.last().unwrap().0 + delta_key,
                src.last().unwrap().1 + (delta_value1 + delta*delta_key)*delta_key,
                Interpolation::CatmullRom,
            ));
        };
        add_value(1.);
    ...
    /// from vector of the key - value pairs
    /// Values must be sorted by key
    pub fn new_cosine(src: &[(f64, f64)]) -> Result<Curve, Error> {
        if src.len() <= 2 {
            return Err(Error::FromString(
