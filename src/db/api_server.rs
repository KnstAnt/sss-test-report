...
impl ApiServer {
 ...
 /*   pub fn get_criterion_data(&mut self) -> Result<DataRowArray, Error> {
        DataRowArray::parse(
            &self
            .fetch(&format!(
                "SELECT criterion_id AS id, actual_value AS result, limit_value AS target FROM criterion_values WHERE ship_id={} AND project_id IS NOT DISTINCT FROM {};",
                self.ship_id,
                self.project_id,
            ))
            .map_err(|e| {
    ...
    //
    pub fn get_parameters_data(&mut self) -> Result<DataRowArray, Error> {
        DataRowArray::parse(
            &self
                .fetch(&format!(
                    "SELECT parameter_id as id, result FROM parameter_data WHERE ship_id={} AND project_id IS NOT DISTINCT FROM {};",
                    self.ship_id,
                    self.project_id,
                ))
                .map_err(|e| {
